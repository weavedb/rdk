...
class Snapshot {
  ...
  async delete(contractTxId) {
    try {
      const zip = path.resolve(this.cacheDirPath, `${contractTxId}.zip`)
      const src = path.resolve(
        this.cacheDirPath,
        `${contractTxId}-downloaded.zip`,
      )
      const dest = path.resolve(this.cacheDirPath, `${contractTxId}/`)
  ...
  async recover(contractTxId, suffix) {
    ...
    await this._recover(contractTxId, suffix)
  }
  ...
  async _recoverRedis(redis, contractTxId, client, suffix) {
    ...
    const src = path.resolve(this.cacheDirPath, destination)
    let exist = false
    try {
      if (!isNil(this.gcsBucket)) {
  ...
  async uploadToGCSRedis(contractTxId, redis, suffix) {
    ...
    const json_path = path.resolve(this.cacheDirPath, destination)
  ...
  async uploadToGCS(contractTxId, suffix) {
    ...
    await this.gcsBucket.upload(path.resolve(this.cacheDirPath, destination), {
      destination,
    })
  }
  ...
  archive(contractTxId, uploader, suffix) {
    ...
    archive.pipe(output)
    archive.directory(path.resolve(this.cacheDirPath, `warp/`), "warp")
    archive.directory(path.resolve(this.cacheDirPath, `rollup/`), "rollup")
    archive.directory(path.resolve(this.cacheDirPath, `wal/`), "wal")
    archive.finalize()
  }
}
