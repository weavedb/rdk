...
contract MultiZKRollup is ZKQuery {
  address public verifierRU;
  address public committer;
  mapping(string => uint) public roots;
  function _validateQueryRU(string memory txid, uint[] memory path, uint[] memory zkp, uint size_path, uint size_val) internal view returns(uint[] memory){
    require(zkp[size_path + size_val + 9] == roots[txid], "root mismatch");
  ...
  function commit (string memory txid, uint[] memory zkp) public returns (uint) {
    require (zkp[9] == roots[txid], "wrong merkle root");
